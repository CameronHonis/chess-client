...
describe("parseMessageFromJson", () => {
    it("parses a MatchUpdatedMessage", () => {
        ...
        expect(parsedMsg.content.match).toBeInstanceOf(Match);
        const expMatch = new Match(Match.template() as Match);
        ...
        expect(parsedMsg.content.match.timeControl).toBeInstanceOf(TimeControl);
    });
});