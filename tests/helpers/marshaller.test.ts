import {Marshaller, Unmarshallable} from "../../src/helpers/marshaller";

class Company extends Unmarshallable {
    name: string;
    employees: Employee[];
    contact: ContactInfo;
...
class Employee extends Unmarshallable {
    id: string;
    name: string;
    age: number;
    isRemote: boolean;
...
class ContactInfo extends Unmarshallable {
    email: string;
    phone: string;
...
describe("Marshaller", () => {
    describe("unmarshalJson", () => {
        it("returns an instance of the given class", () => {
            const out = Marshaller.unmarshalJson(defaultCompanyJson(), Company);
        ...
        describe("when the JSON is missing a property", () => {
            ...
            it("throws a type error", () => {
                expect(() => {
                    Marshaller.unmarshalJson(companyJson, Company);
                }).toThrow(TypeError);
            });
        ...
        describe("when the JSON has an unexpected property", () => {
            ...
            describe("when strict mode is enabled", () => {
                it("throws a type error", () => {
                    expect(() => {
                        Marshaller.unmarshalJson(companyJson, Company, true);
                    }).toThrow(TypeError);
                });
            })
            describe("when strict mode is disabled", () => {
                it("does not throw an error", () => {
                    expect(() => {
                        Marshaller.unmarshalJson(companyJson, Company, false);
                    }).not.toThrow();
                });
            })
        })
